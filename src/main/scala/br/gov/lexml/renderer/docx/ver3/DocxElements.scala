...
case object NoBreakHypher extends XmlComponent {
  val asXML = <w:noBreakHyphen/>
}

case class Sym(font : String, char : String) extends XmlComponent {
  val asXML = <w:sym w:font={font} w:char={char}/>
}

case class T(text : String, preserveSpace : Boolean = false) extends XmlComponent {
  lazy val asXML = <w:t xml:space={if(preserveSpace) { "true" } else {  null }}>{text}</w:t>
}

case object Tab extends XmlComponent {
  val asXML = <w:tab/>
}
...
case class PTab(
    ...
    relativeTo : PTabBase) extends XmlComponent {
  val asXML = <w:ptab/>
}

final case class ContentPar(id : String) extends XmlComponent {
